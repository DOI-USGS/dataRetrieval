...
This article highlights a "scripting" approach, meaning it will only use basic R techniques. There is less of a learning curve in this approach, but also if something goes wrong, it is more challenging to re-run. The alternative is the pipeline approach, which is highlighted in the article [Large Data Pulls from Water Quality Portal - A Pipeline-Based Approach](articles/wqp_large_pull_targets.html).
