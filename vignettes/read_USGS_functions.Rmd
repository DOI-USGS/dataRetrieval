...
The `read_USGS_dv` function will eventually replace the `readNWISdv` function. Generally, both functions look pretty similar:
...
dv_modern <- read_USGS_dv(monitoring_location_id = "USGS-01491000",
                          parameter_code = c("00060", "00010"),
                          statistic_id = "00003",
                          datetime = c("2023-01-01", "2025-05-06"))
...
```{r}
dv_2 <- read_USGS_dv(monitoring_location_id = "USGS-01491000",
                     parameter_code = c("00060"),
                     properties = c("monitoring_location_id",
                                    "value",
                                    "time"),
                     statistic_id = "00003",
                     datetime = c("2022-10-01", "2023-10-01"))
head(dv_2)
```

* When you look at the help file for the new function `?read_USGS_dv`, you'll also notice there are many more arguments. These are mostly set by default to NA. You DO NOT need to specify all of these parameters.
...
To access these services on a web browser, go to <https://api.waterdata.usgs.gov/ogcapi/v0/collections/timeseries-metadata>.
...
The `read_USGS_sites` function will eventually replace the `readNWISsite` function. Generally, both functions look pretty similar:
...
```{r}
sites_modern <- read_USGS_sites(monitoring_location_id = "USGS-01491000")
names(sites_modern)
