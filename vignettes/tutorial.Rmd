...
*  `reaWQPsummary` summarizes the data available within the WQP by year.
...
For example, let's see which sites ever measured phosphorus in Arizona. Water quality data is exclusively found in WQP functions. 
...
```{r echo=FALSE, eval=TRUE}
library(dplyr)
AZdata2 <- az_phos_summary |> 
  select(Site, min_year, max_year, count, POR)
...
Let's do one more example, we'll look for long-term USGS phosphorous data in Wisconsin. This time, we will take the information from the `readWQPsummary` function, filter down the sites to exactly our interest, and then get the data. Let's say we want data from sites that have been collecting data for at least 15 years and have at least 300 measurements:
...
wi_phos_data <- readWQPdata(siteid = wi_phos_summary$Site,
                            characteristicName = "Phosphorus",
                            dataProfile = "narrow")

```
```{r echo=FALSE}
wi_phos_data <- readRDS("wi_phos_data.rds")
```

Let's look at the maximum measured value, and number of samples:

```{r}
siteInfo <- attr(wi_phos_data, "siteInfo")

phos_summary <- wi_phos_data |> 
  group_by(Location_Identifier, Location_Name, 
           Location_Latitude, Location_Longitude) |> 
  summarize(max = max(as.numeric(Result_Measure)),
            count = n())
```
...
phosData <- readWQPdata(
  siteid = phos_data_sites_to_get$MonitoringLocationIdentifier,
  characteristicName = "Phosphorus"
)
```
