...
## Supplemental Slides: Multiple parameter example

```{r}
pCode <- c("00665","80154") #Phos and Sed
sitesDC <- readNWISdata(stateCd="DC", service="site",
                     seriesCatalogOutput=TRUE)
sitesDC.1 <- filter(sitesDC, parm_cd %in% pCode) %>%
            filter(count_nu > 300)

comboData <- readNWISqw(sitesDC.1$site_no, pCode, reshape = TRUE)
comboData <- renameNWISColumns(comboData)
```
 
## Supplemental Slides: Multiple parameter example (cont.)
...
plot(comboData$result_va_80154,comboData$result_va_00665,log="xy")
mod <- lm(log(result_va_00665)~log(result_va_80154),data=comboData)
print(summary(mod))
plot(comboData$sample_dt,mod$residuals)
abline(h=0)
