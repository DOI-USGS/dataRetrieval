...
`dataRetrieval` includes a data set `parameterCdFile` that allows you to explore the USGS parmeter codes. For example, to find all the parameter codes with the word "phosphorus" in the name:
...
```{r echo=FALSE, eval=TRUE}
library(DT)
datatable(phosCds, rownames = FALSE,options = list(pageLength = 5))
```
