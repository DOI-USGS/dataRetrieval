...
test_that("readNWISstat tests", {
  testthat::skip_on_cran()
  data <- readNWISstat(siteNumbers=c("02171500", NA),parameterCd=c("00010","00060"),
...
test_that("readNWISuse tests", {
  ...
  national <- readNWISuse(stateCd=NULL, countyCd = NULL, years="all", transform = TRUE)
  expect_is(national$Year,'integer')
})
